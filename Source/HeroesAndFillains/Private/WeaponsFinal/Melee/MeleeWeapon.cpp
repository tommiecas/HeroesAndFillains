...
{
	EMeleeState MeleeWeaponState = State;

	switch (MeleeWeaponState)
	{
	case EMeleeState::EMS_Equipped:
		...
		if (PickupWidgetA)
		{
			PickupWidgetA->DestroyComponent();
		}
		if (PickupWidgetB)
		{
			PickupWidgetB->DestroyComponent();
		}
		if (MeleeInfoWidget1)
		{
			MeleeInfoWidget1->DestroyComponent();
		}
		if (MeleeInfoWidget2)
		{
			MeleeInfoWidget2->DestroyComponent();
		}
		break;
