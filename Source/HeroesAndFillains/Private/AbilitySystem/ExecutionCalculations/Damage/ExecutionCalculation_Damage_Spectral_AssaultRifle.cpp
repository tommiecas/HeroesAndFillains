...
#include "AbilitySystem/ExecutionCalculations/ExecutionCalculation_Damage_Spectral_AssaultRifle.h"
...
UExecutionCalculation_Damage_Spectral_AssaultRifle::UExecutionCalculation_Damage_Spectral_AssaultRifle()
{
...
void UExecutionCalculation_Damage_Spectral_AssaultRifle::Execute_Implementation(
	...
	FGameplayEffectCustomExecutionOutput& ExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FHAFGameplayTags::Get().DamageTypesToResistances)
	{
		...
		ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(CaptureDef, EvaluationParameters, Resistance);
	...
	ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(HAFDamageStatics().BlockChanceDef, EvaluationParameters, TargetBlockChance);
	...
	ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(HAFDamageStatics().ArmorDef, EvaluationParameters, TargetArmor);
	TargetArmor = FMath::Max<float>(TargetArmor, 0.f);

	float SourceArmorPenetration = 0.f;
	ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(HAFDamageStatics().ArmorPenetrationDef, EvaluationParameters, SourceArmorPenetration);
	...
	ExecutionOutput.AddOutputModifier(EvaluatedData);
	...
	ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(HAFDamageStatics().CriticalHitChanceDef, EvaluationParameters, SourceCriticalHitChance);
	SourceCriticalHitChance = FMath::Max<float>(SourceCriticalHitChance, 0.f);

	float SourceCriticalHitDamage = 0.f;
	ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(HAFDamageStatics().CriticalHitDamageDef, EvaluationParameters, SourceCriticalHitDamage);
	SourceCriticalHitDamage = FMath::Max<float>(SourceCriticalHitDamage, 0.f);

	float TargetCriticalHitResistance = 0.f;
	ExecutionParameters.AttemptCalculateCapturedAttributeMagnitude(HAFDamageStatics().CriticalHitResistanceDef, EvaluationParameters, TargetCriticalHitResistance);
	...
	ExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(
